...
impl Actor {
	...
	pub fn render(&self, renderer: &mut Renderer) {
		renderer.copy_ex(&self.texture, None, Some(Rect::new(self.position[0] as i32, self.position[1] as i32, self.texture.query().width, self.texture.query().height)), self.rotation as f64, None, false, false).expect("Write to screen failed.");
	}
}